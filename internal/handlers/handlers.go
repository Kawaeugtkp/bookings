...
import (
	...
	"github.com/Kawaeugtkp/bookings/internal/forms"
	...
	"github.com/Kawaeugtkp/bookings/internal/repository"
	"github.com/Kawaeugtkp/bookings/internal/repository/dbrepo"
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	start := r.Form.Get("start_date") // inputのnameの方のstartとmatchしている
...
func (m *Repository) PostAvailability(w http.ResponseWriter, r *http.Request) {
	start := r.Form.Get("start") // inputのnameの方のstartとmatchしている
	end := r.Form.Get("end")
	layout := "2006-01-02"
	startDate, err := time.Parse(layout, start)
	if err != nil {
...
func (m *Repository) AvailabilityJSON(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Println("errrrrrrrrrrrrrrrrrr1")
	}
	endDate, err := time.Parse(layout, ed)
	if err != nil {
		log.Println("errrrrrrrrrrrrrrrrrr2")
	}

	roomID, err := strconv.Atoi(r.Form.Get("room_id"))
	if err != nil {
		log.Println("errrrrrrrrrrrrrrrrrr3")
	}

	available, err := m.DB.SearchAvailabilityByDatesByRoomID(startDate, endDate, roomID)

	if err != nil {
		log.Println("errrrrrrrrrrrrrrrrrr4")
	}
	...
	} // これがgeneralsとかの追加されたjsのcallback内のdataの部分に該当しているから、
	// data.okとかいう呼び方ができる
...
func (m *Repository) ReservationSummary(w http.ResponseWriter, r *http.Request) {
	reservation, ok := m.App.Session.Get(r.Context(), "reservation").(models.Reservation)
	// .(models.Reservation)の部分はGetで取り出しているもののtypeを
	// 教えてあげているって考えればいい。で、このtypeがちゃんと
	// models.Reservationであればokがtrueになってそうでなければfalseになる。
	// 例えばユーザーが直接reservation-summaryとかをurlに入力した場合には
	// ここがfalseになる
...
// BookRoom takesURL parameters, builds a sessional variable, and takes user 
// to make res screen
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	...
	form.IsEmail("email") // IsEmailの引数は関数内でForm.Getの引数として
	// 使われるから、この場合普通に上で作ったemail propertyは使ってはいけない
	if !form.Valid() {
		render.Template(w, r, "login.page.html", &models.TemplateData{
			Form: form,
		})
	...
	// ここがflashになっていたが、もしかしたらinstractorが動画作成した時期には
	// notieに存在していたflashが今は形を変えてしまったのかもしれない
	http.Redirect(w, r, "/", http.StatusSeeOther)
}
...
// AdminShowReservationshows the reservation in the admin tool
func (m *Repository) AdminShowReservation(w http.ResponseWriter, r *http.Request) {
	...
	year := r.URL.Query().Get("y") // これってGetしようとしたものが存在しなくても書いていていいコードみたい
