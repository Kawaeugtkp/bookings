...
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int, error) {
	...
	stmt := `insert into reservations (first_name, last_name, email, phone, 
		start_date, end_date, room_id, created_at, updated_at) values($1, 
...
func (m *postgresDBRepo) InsertRoomRestriction(r models.RoomRestriction) error {
	...
	stmt := `insert into room_restrictions (start_date, end_date, room_id, 
		reservation_id, created_at, updated_at, restriction_id) values($1, 
		$2, $3, $4, $5, $6, $7)`
	
	_, err := m.DB.ExecContext(ctx, stmt, r.StartDate, r.EndDate, r.RoomID, 
...
// SearchAvailabilityByDatesByRoomID returns true if availability existes for roomID, 
// and false if no aavailability
...
func (m *postgresDBRepo) SearchAvailabilityForAllRooms(start, end time.Time) ([]models.Room, error) {
	...
	from
		rooms r
	where
		r.id not in (select rr.room_id from room_restrictions rr where 
...
// GetUserByIDreturn a user by id
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `select id, first_name, last_name, email, password, 
		access_level, created_at, updated_at from users where id=$1`

	row := m.DB.QueryRowContext(ctx, query, id)

	var u models.User
	err := row.Scan(&u.ID, &u.FirstName, &u.LastName, &u.Email, &u.Password, 
	&u.AccessLevel, &u.CreatedAt, &u.UpdatedAt)
...
func (m *postgresDBRepo) UpdateUser(u models.User) error {
	...
	_, err := m.DB.ExecContext(ctx, query, u.FirstName, u.LastName, 
		u.Email, u.AccessLevel, time.Now())
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	row := m.DB.QueryRowContext(ctx, `select id, password from users where 
	email = $1`, email)
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	var reservartions []models.Reservation
	...
	query := `
		...
		rm.id, rm.room_name from reservations r left join rooms rm on
		(r.room_id = rm.id) order by r.start_date asc`

	rows, err := m.DB.QueryContext(ctx, query)
	if err != nil {
		return reservartions, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		reservartions = append(reservartions, i)
	}
	if err = rows.Err(); err != nil {
		return reservartions, err
	}
	return reservartions, nil
}
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	query := `
		...
		rm.id, rm.room_name from reservations r left join rooms rm on
		(r.room_id = rm.id) where processed = 0 order by r.start_date asc`
...
func (m *postgresDBRepo) GetReservationByID(id int) (models.Reservation, error) {
	...
	query := `
		...
		r.created_at, r.updated_at, r.processed, rm.id, rm.room_name from reservations r left join rooms rm
		on (r.room_id = rm.id) where r.id = $1`
...
func (m *postgresDBRepo) AllRooms() ([]models.Room, error) {
	...
	for rows.Next() {
		var rm models.Room
		err := rows.Scan(
			&rm.ID, &rm.RoomName, &rm.CreatedAt, &rm.UpdatedAt,
		)
		if err != nil {
			return rooms, err
		}
...
func (m *postgresDBRepo) GetRestrictionsForRoomByDate(roomID int, start, end time.Time) ([]models.RoomRestriction, error) {
	...
	query := `
		select id, coalesce(reservation_id, 0), restriction_id, room_id, start_date, end_date from 
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&r.ID, &r.ReservationID, &r.RestrictionID, &r.RoomID, &r.StartDate, &r.EndDate,
		)
		if err != nil {
			return nil, err
		}
